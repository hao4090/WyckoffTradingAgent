...
| 功能 | 说明 |
|------|------|
| 💬 **读盘室** | 首页即为对话界面，基于 Google ADK 构建，威科夫本人坐镇——用自然语言让他亲自审视盘面、筛选、诊断、下达去留指令，支持上下文记忆 |
...
| 🧭 **沙里淘金** | Web 前台提交参数，GitHub Actions 在后台执行多层漏斗筛选（高级参数调优入口，日常筛选推荐走读盘室对话） |
| 🤖 **大师模式** | 单股本地深度分析（默认近 320 个交易日，含图表生成）；批量代码改为后台研报任务 |
...
| 📁 **数据导出** | A 股日线导出 + 自定义导出（ETF/指数/宏观 CPI）+ 下载历史，统一入口 |
| 💼 **持仓管理** | 实时同步持仓至云端并生成状态签名；内置 AI 订单建议面板、自动作废旧单与过时预警 |
| 🎯 **推荐跟踪** | 记录历史推荐股票及加入价，自动同步每日收盘价并计算累计收益 |
| 📣 **市场信号栏** | 登录后页面顶部统一展示最新交易日的大盘水温、A50（盘前风向标）与 VIX（恐慌指数） |
| 🔐 **登录与配置** | 支持登录；飞书 / 企微 / 钉钉 Webhook 及多厂商 API Key 云端同步 |
...
1. **智能对话（Web 首页）** — 真正的 AI Agent。基于 **Google ADK** 构建，LLM 自主理解意图、选择工具、串联多步推理。
2. **定时批处理 Pipeline（GitHub Actions）** — 确定性流水线。固定 5 阶段串行执行，其中 2 个阶段调用 LLM 生成文本，本质是带重试的 ETL 任务链，不涉及自主决策。

### 智能对话 Agent（Google ADK）

首页即为 AI 对话界面。用户输入自然语言后，ADK 的 `LlmAgent` 自主判断意图、决定调哪些工具、调几次，并根据中间结果动态调整后续动作。
...
└────────────────────────┬────────────────────────────┘
                         ...
                         ▼
              ┌─────────────────────┐
              │  ChatSessionManager │ ← 同步包装 ADK 异步 Runner
              ...
              │   (Gemini 2.0)     │    LLM 自主选择工具 + 多步推理
              └──────────┬──────────┘
                         │ FunctionTool 自动调度（LLM 决定调哪些、调几次）
          ┌──────┬───────┼───────┬──────┬──────────┐
          ▼      ▼       ▼       ▼      ▼          ▼
      搜索股票 个股诊断 持仓诊断 行情查询 大盘概览  漏斗筛选
                                                    │
                                              ┌─────┴──────┐
                                              ▼            ▼
                                          AI 研报    策略决策
...
**为什么这是真正的 Agent**：用户说"帮我看看 000001 和 600519 哪个更值得买"，LLM 会自主决定先调 `diagnose_stock` 两次、再调 `get_stock_price` 对比行情、最后综合推理给出结论。整个过程无需预编排，工具调用顺序和次数由 LLM 实时决策。

### 对话工具清单（9 个 FunctionTool）

ADK 的 `FunctionTool` 自动从 Python 函数的 type hints + docstring 生成工具 schema，**无需装饰器**。

| 工具函数 | 说明 | 复用的核心模块 |
|---------|------|---------------|
| `search_stock_by_name` | 名称/代码/拼音模糊搜索 | tushare `stock_basic` |
| `diagnose_stock` | 单股 Wyckoff 结构化健康诊断 | `core.holding_diagnostic` |
| `diagnose_portfolio` | 批量持仓诊断 | Supabase 持仓 + `holding_diagnostic` |
| `get_stock_price` | 近期 OHLCV 行情查询 | `integrations.data_source` |
| `get_market_overview` | 主要指数涨跌幅概览 | tushare `index_daily` |
| `screen_stocks` | Wyckoff 五层漏斗全市场筛选 | `core.funnel_pipeline` |
| `generate_ai_report` | 三阵营 AI 深度研报 | `core.batch_report` |
| `generate_strategy_decision` | 持仓去留 + 新标的买入决策 | `core.strategy` + OMS |
| `get_recommendation_tracking` | 历史推荐记录及后续表现 | `supabase_recommendation` |

### 会话管理

| 层级 | 实现 | 说明 |
|------|------|------|
| Session | `InMemorySessionService` (Phase 1) | 进程内存储，重启丢失；后续可切 `DatabaseSessionService` 连 Supabase PG |
| State | `session.state` | 存储 `user_id`、偏好 provider 等上下文 |
| Memory | 待接入 | 可升级为 `VertexAiMemoryBankService` 或自建 Supabase 实现 |

### 定时批处理 Pipeline（GitHub Actions）

定时全市场筛选走固定 5 阶段串行流水线（`.github/workflows/wyckoff_funnel.yml`），本质是**确定性 ETL 任务链 + 两次 LLM 调用**，不涉及自主决策：

```
触发 (cron 北京时间周日~周四 18:25 / 手动)
  │
  ├─ [1] 漏斗筛选 ──── 拉取 ~4500 只 A 股 OHLCV → 4 层过滤 → ~30 只候选
  │
  ├─ [2] 大盘水温 ──── 计算 regime (RISK_ON / NEUTRAL / RISK_OFF / CRASH)
  │
  ├─ [3] LLM 研报 ──── 拼 prompt + 调一次 Gemini → 三阵营 Markdown 研报
  │
  ├─ [4] LLM 决策 ──── 拼 prompt + 调一次 Gemini → 持仓去留 JSON → OMS 风控
  │
  └─ [5] 通知推送 ──── 飞书 + 企微 + 钉钉 + Telegram
```

每个阶段由 `scripts/daily_job.py` 串行调用，失败重试后继续下一步。

| 阶段 | 代码模块 | 本质 |
|------|---------|------|
| 漏斗筛选 | `scripts/wyckoff_funnel.py` → `core/wyckoff_engine.py` | 确定性计算（均线/量价/阈值过滤） |
| 大盘水温 | `scripts/wyckoff_funnel.py` benchmark 部分 | 确定性计算（指数趋势 + 市场广度） |
| LLM 研报 | `scripts/step3_batch_report.py` | 拼 prompt → 单次 LLM 调用 |
| LLM 决策 | `scripts/step4_rebalancer.py` | 拼 prompt → 单次 LLM 调用 |
| 通知推送 | 飞书/企微/钉钉/Telegram webhook | HTTP POST |

### LLM Provider 支持

智能对话默认使用 Gemini（ADK 原生支持）；定时 Pipeline 通过 LiteLLM 统一适配层支持多 provider：

| Provider | 状态 | 说明 |
|----------|------|------|
| Gemini | ✅ 主力 | 对话 Agent 默认 `gemini-2.0-flash` |
